...
- Бизнес-сценарий:
  Сотрудник логистики, перевозчик или партнёр входит в систему, чтобы работать с окнами приёма и своими бронями.
- Цель:
...
- Бизнес-сценарий:
  Диспетчер или клиент открывает список окон приёма и должен сразу увидеть, какие окна ещё доступны и сколько мест осталось в каждом.
- Цель:
...
- Бизнес-сценарий:
  Пользователь начал оформление поставки или визита и временно занимает место в выбранном окне, чтобы его не забрал другой участник процесса.
- Цель:
...
- Бизнес-сценарий:
  Пользователь или интеграция случайно отправили одно и то же действие повторно, и система не должна создать дубль брони.
- Цель:
...
- Бизнес-сценарий:
  Все места в окне уже почти заняты, и очередной запрос не должен переполнить окно приёма сверх его реальной вместимости.
- Цель:
...
- Бизнес-сценарий:
  Временная бронь завершена и должна стать окончательной, чтобы окно было закреплено за конкретной поставкой или визитом.
- Цель:
...
- Бизнес-сценарий:
  Пользователь слишком долго не завершал оформление, временная бронь устарела, и место должно вернуться в общий пул доступности.
- Цель:
...
- Бизнес-сценарий:
  Поставка или визит отменились, и система должна сразу вернуть ранее занятое место обратно в доступные окна.
- Цель:
...
- Бизнес-сценарий:
  Посторонний пользователь без входа в систему не должен иметь возможности менять загрузку окон приёма.
- Цель:
...
- Бизнес-сценарий:
  Один и тот же клиент или перевозчик оформляет несколько независимых поставок в одно окно, и система должна это поддержать, пока вместимость позволяет.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что пользователь может войти в систему на целевом серверном стеке и начать работу с окнами приёма.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что на целевом стеке диспетчер видит актуальную картину доступных окон и остатков по ним.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что временное удержание места в окне корректно работает уже на рабочем SQL runtime сервера.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что случайный повтор того же действия не создаёт вторую бронь и не искажает загрузку окна на целевом стеке.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что система не допустит переполнения окна приёма даже на реальной серверной БД.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что несколько отдельных поставок одного клиента могут жить в одном окне как независимые брони, если ещё есть место.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что временное удержание корректно становится окончательной бронью на целевом стеке.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что отменённая поставка сразу освобождает место в окне приёма на целевом стеке.
- Цель:
...
- Бизнес-сценарий:
  Повторно подтверждаем, что без авторизации никто не может повлиять на загрузку окон и бронирования на целевом стеке.
- Цель:
...
- Бизнес-сценарий:
  После цепочки действий бизнес должен увидеть логически правильный итог: подтверждённые места остаются занятыми, отменённые возвращаются в доступность.
- Цель:
